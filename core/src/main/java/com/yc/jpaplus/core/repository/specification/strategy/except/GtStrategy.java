 ...
 */
public class GtStrategy implements IExceptStrategy {
    @Override
    public Specification except(DtoFieldWrapper wrapper) {
        ...
        if(value instanceof Comparable) {
            ...
            if(StringUtils.isNotEmpty(wrapper.getJoinName())){
                return (root, query, cb) ->cb.greaterThan(root.join(wrapper.getJoinName()).get(wrapper.getFieldName()), val);
            }
            return (root, query, cb) -> cb.greaterThan(root.get(wrapper.getFieldName()), val);
        }else {
